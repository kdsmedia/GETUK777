...
- **ResultRow → domain**: `fun ResultRow.toX(): X` as an extension inside `object XMapper`. Distinct name (e.g. `toCasinoProvider()`, `toAggregator()`) rather than `toDomain()` because multiple mappers defining `ResultRow.toDomain()` collide when one mapper composes another (`CasinoProviderMapper` calls `AggregatorMapper.toAggregator()` on a joined row).
...
- ResultRow extensions named `toX()` where `X` is the aggregate name (`toCasinoGame`, `toCasinoProvider`, `toCollection`, `toAggregator`).
...
One mapper file per aggregate/entity (e.g. `CasinoGameMapper.kt`, `CasinoSessionMapper.kt`). Don't group unrelated mappers in a single file.
...
- `infrastructure/persistence/mapper/CasinoGameMapper.kt`
- `infrastructure/persistence/mapper/CasinoSessionMapper.kt`
- `infrastructure/persistence/mapper/CasinoRoundMapper.kt`
