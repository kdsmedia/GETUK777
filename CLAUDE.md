...
The client jar is built with a newer Kotlin than this module reads natively, so `build.gradle.kts` sets `-Xskip-metadata-version-check`. Launch URLs put the game in a subdomain (`<gameSymbol>.<gameHost>`) and the mode in the query: demo → `?mode=demo`, real → `?mode=real&sessionToken=<token>&operatorIdentity=<id>`. The `sessionToken` for real launches comes from a live `SessionService.CreateSession` call. Config keys: `address` (provider gRPC `host:port`), `operatorIdentity`, `apiKey`, `gameHost`.
