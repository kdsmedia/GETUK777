...
- **Outbound (we → provider)**: `TongameHttpClient` (`infrastructure/aggregator/tongame/client/`, Ktor CIO, `expectSuccess=true`) calls `GET <apiUrl>/api/v1/operator/game` and `POST <apiUrl>/api/v1/operator/session` with `X-Identity` + `X-Api-Key` headers. `getAggregatorGames()` maps the game list → `AggregatorGame` (provider supplies `identity`/`name`; locales/platforms defaulted). **We mint the session token**: `getLaunchUrl()` registers our own `session.token` via `POST /session` `{sessionToken, playerId, gameId}` (returns 204) and embeds that same token in the launch URL — so the provider echoes it back in wallet webhooks and it resolves via our `findByToken`. Freespins unsupported.
- **Inbound (provider → us)**: aggregator **webhook** like `OneGameHubWebhook`/`PragmaticWebhook`. `TongameWebhook` (`infrastructure/aggregator/tongame/webhook/`) exposes four flat POST routes under `/api/webhook/tongame` — `/balance`, `/place`, `/settle`, `/closeRound` — bound in `aggregatorModule` and wired into `api/webhook/WebhookModule.kt`'s `configureWebhook()` (all webhooks share the Ktor HTTP server; **no separate gRPC server**). The registered `webhookUrl` is `<our-host>/api/webhook/tongame`. Each route resolves **our** session via `FindSessionQuery(token)` and bridges into the spin pipeline via `Bus`: `/balance`→`FindSessionBalanceQuery`, `/place`→`PlaceSpinSessionCommand`, `/settle`→`SettleSpinSessionCommand`, `/closeRound`→`EndRoundSessionCommand`+balance. **Currency is not locked to the session for TONGame** — the player can switch currency in-game, so every wallet call carries its own `currency`. The webhook pins it onto the resolved session (`session.copy(currency = …)`) before dispatching, so the wallet op hits the right currency (see the session-command contract below). Round opening is lazy (first `/place`); spin ids are synthesized from `roundId` (`<roundId>:place` / `:settle`). Money on the wire is a **decimal string in whole currency units** (e.g. `"10.5"` TON, not nano) — the webhook converts to/from the wallet's system units via `ICurrencyPort` (`convertToUnits` inbound on `/place`+`/settle`, `convertFromUnits` for the balance response). Domain exceptions map to HTTP status per the contract: `SessionNotFound`→`401`, `Forbidden`/`NotFound`/`Conflict`→`409`; anything else propagates (→500).

Launch URLs put the game in a subdomain (`<gameSymbol>.<gameHost>`) and the mode in the query: demo → `?mode=demo`, real → `?mode=real&sessionToken=<token>&operatorIdentity=<id>`. Config keys: `apiUrl` (provider REST base, e.g. `https://provider.example.com`), `operatorIdentity`, `apiKey`, `gameHost`.
