 ...
 * or `CasinoGameNotFoundException` if the game is not currently a member of the
 * collection. The caller is responsible for avoiding collisions — the read
 * side breaks ties on `CasinoGameTable.id`.
 */
data class UpdateCollectionCasinoGameOrderCommand(
